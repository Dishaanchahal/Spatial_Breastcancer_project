...
In this study we combined reference-based deconvolution with spatial tissue analysis to dissect the tumour microenvironment of six breast cancers, and then asked whether the resulting picture held up in independent spatial data and in large clinical cohorts. Mapping the Wu et al. single-cell reference onto the Visium spots with cell2location resolved the nine major cell populations per spot, and an independent deconvolution with RCTD reproduced these compositions closely (Pearson r = 0.93), giving us confidence that the estimates reflect biology rather than the idiosyncrasies of a single algorithm. The two clinical subtypes separated as expected: the oestrogen-receptor-positive tumours were dominated by malignant epithelium and largely devoid of immune cells, whereas the triple-negative tumours were more heavily infiltrated by cancer-associated fibroblasts (CAFs) and immune populations, and one triple-negative case contained a compact B- and T-cell aggregate with the hallmarks of a tertiary lymphoid structure (TLS).

Several independent analyses converged on the same organising axis. Ligand–receptor inference nominated a CAF-derived CXCL12–CXCR4 interaction as a specific route by which the stroma engages T and B cells, and both the neighbourhood-based niche analysis and a non-negative matrix factorisation of the cell-type abundances independently recovered a co-occurring lymphoid (B + T) community alongside a distinct CAF/myeloid stromal community. Pathway scoring placed JAK-STAT activity within the lymphoid compartment and TGF-β activity within the CAF stroma. Read together, these results describe a coherent circuit in which CAFs recruit lymphocytes through CXCL12, those lymphocytes organise into a TLS, and the whole is embedded in a TGF-β-rich stromal compartment. This interpretation is consistent with a substantial body of work: CAF-derived CXCL12 is an established chemoattractant and retention signal for lymphocytes ([Costa et al., 2018](https://consensus.app/papers/details/8657f2141aec5457b021d011a80352e2/); [Kudo et al., 2025](https://consensus.app/papers/details/e1c0a61038df5214afe0fd249954eb97/)), and TGF-β signalling in fibroblasts is a canonical driver of T-cell exclusion and immunotherapy resistance ([Mariathasan et al., 2018](https://consensus.app/papers/details/d8aa01ae7bac567e9ab207dc9401c8da/)).

The clinical reading of these compartments was borne out in survival data. A TLS gene signature was associated with better outcome in both TCGA (recurrence endpoints) and, most convincingly, in the well-powered METABRIC cohort (overall-survival HR 0.87 per standard deviation, p = 3 × 10⁻⁵), reproducing the favourable prognostic value of TLS reported in breast-cancer meta-analyses ([Zhang et al., 2021](https://consensus.app/papers/details/5611b74430de5147987dc696af0da49d/)). Conversely, higher stromal TGF-β activity tracked with worse recurrence-free outcome. These associations position the lymphoid/TLS programme as a candidate prognostic marker and the TGF-β/CAF stroma as a candidate therapeutic target — both of which are being actively pursued in the clinic (for example, TGF-β blockade combined with checkpoint inhibition, and CXCR4 antagonism).

The principal strength of the study is triangulation. Every major conclusion rests on at least two independent computational approaches (anchor-based label transfer and RCTD for composition; niche clustering, NMF microenvironments and a TLS signature for tissue architecture), is reproduced in an external Visium dataset that the pipeline was never trained on, is associated with outcome in two clinical cohorts, and is concordant with the published literature. This layering makes it unlikely that the central findings are artefacts of any single method or dataset.

The work also has clear limitations that temper the conclusions. The spatial cohort is small (six sections from a single atlas), so the subtype contrasts are descriptive rather than definitive. Deconvolution is reference-dependent: the cell2location estimates over-called a "Normal Epithelial" fraction that RCTD largely reassigned to malignant epithelium, so fine epithelial subclassification should be treated with caution, and inferring malignancy directly (for example with copy-number methods) would be a useful addition. Because Visium spots are 55 µm and therefore multicellular, the co-occurrence we describe is spot-level rather than true single-cell colocalisation, and the CXCL12–CXCR4 interaction is inferred from reference co-expression rather than measured in situ; a spatially resolved communication method would be needed to confirm it in tissue. We deliberately did not claim JAK-STAT as prognostic, because it trended protective in TCGA but did not replicate in METABRIC — behaviour that is expected given the opposing STAT1/interferon and STAT3/IL-6 arms of the pathway. Finally, the survival analyses are retrospective and signature-based rather than based on histologically defined TLS, the TNBC-restricted analyses were underpowered, and METABRIC was accessed through a public mirror of the cBioPortal study.

Future work should aim to move from association towards mechanism and from description towards prediction: spatially resolved ligand–receptor analysis to confirm the CXCL12 axis in tissue, finer-grained deconvolution to resolve CAF subsets (myofibroblastic versus inflammatory) and T-cell states, dedicated image-based TLS detection, higher-resolution platforms such as Visium HD or Xenium, and ultimately prospective testing of TLS as a predictive biomarker and of the TGF-β/CXCR4 stromal axis as a therapeutic target.
